...
static const uint8_t CS_PINS[MAG_SENSOR_NUM] = {21};//,21};//{23, 22, 21, 20, 19, 18, 17, 16, 25, 11};// chip select 
static const uint8_t RDY_PINS[MAG_SENSOR_NUM] = {4};//,3};//{0, 2 , 3, 4, 5, 6, 7, 8, 10, 12};// Data ready
...
void loop()
{
    ...
    if (digitalRead(RDY_PINS[0]) == HIGH){// && digitalRead(RDY_PINS[1]) == HIGH){
